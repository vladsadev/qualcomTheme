...
function banner( $args = [] ): void {
	$pageBannerURL   = get_field( 'imagen_del_banner' );
	$pageBannerImage = $pageBannerURL['sizes']['pageBanner'] ?? get_theme_file_uri( '/images/ocean.jpg' );
	$leyenda         = get_field( 'leyenda_de_pagina' ) ?? ( $args['leyenda_de_pagina'] ?? ' ' );
	?>
    <div class="min-h-72 mt-16 sm:mt-24 py-6 relative md:py-16  lg:py-20 bg-center md:bg-cover bg-black/45 bg-blend-multiply
    text-white text-shadow-md"
        ...
        <div class="container flex h-full flex-col justify-center">
            <h3 class="text-4xl font-thin md:text-5xl xl:text-6xl">
				<?= is_category() ? the_archive_title() : $args['title'] ?? get_field( 'titulo_del_banner' ) ?>
            </h3>
